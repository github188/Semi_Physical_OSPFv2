...
void Sns_Scenarios::All_Node_init()
{
	...
	for (i=0;i<Num_Nodes;i++)
	{
		...
		nodeData[nodelist_id]->Node_Type = SNS_NODE_REAL;					//节点类型枚举
				...
				case SNS_NODE_REAL://接收消息的节点
					{
						...
						else//节点输出事件处理
						{
							...
							case Sns_Node_Real::MSG_OutEVENT_SendToOtherNodes:
								{
									Sim_Test_result->Process_MSG_OutEVENT_SendToOtherNodes(msg);
									break;
								}
								case Sns_Node_Real::MSG_OutEVENT_SendToRealNode:
								{
									Sim_Test_result->Process_MSG_OutEVENT_SendToRealNode(msg);
									break;
								}
								//SNS_MSG_EVENT_OUT//
							default:
